...
Repo.prototype.clone = function (cb) {
  ...
  this.git.exec('clone',null,[url.format(repoUrl), this.worktree], function(err, stdout) {
    if (err) throw(err);
...
Repo.prototype.setUser = function(cb) {
  debug(this.github.repo + ' User: ' + this.name || 'GitPub');
  this.git.exec('config', null, ['user.name',this.name || 'GitPub'], function(err, stdout) {
    if (err) throw(err);
    if (stdout) debug(stdout);
    debug('User Set');
    if (cb) cb();
...
Repo.prototype.setEmail = function(cb) {
  debug(this.github.repo + ' email: ' + this.email);
  this.git.exec('config', null, ['user.email',this.email],function (err, stdout) {
      if (err) debug(err);
      if (stdout) debug(stdout);
      if (cb) cb();
  })
